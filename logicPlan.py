...
def atLeastOne(literals: List[Expr]) -> Expr:
    ...
    return disjoin([e for e in literals])
    "*** END YOUR CODE HERE ***"
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    li = list(combinations(literals,2))
    return conjoin([(~lit[0] | ~lit[1]) for lit in li ])
    "*** END YOUR CODE HERE ***"
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    is_wall = conjoin([PropSymbolExpr(wall_str, coord[0], coord[1]) >> ~PropSymbolExpr(pacman_str, coord[0], coord[1], time = t) for coord in all_coords])
...
def foodLogicPlan(problem) -> List:
    ...
    # making sure food is true at time 0
    for f in food:
...
def localization(problem, agent) -> Generator:
    ...
    wall_loc = [PropSymbolExpr(wall_str, coord[0], coord[1]) if coord in walls_list else ~PropSymbolExpr(wall_str, coord[0], coord[1]) for coord in all_coords]
    KB.append(conjoin(wall_loc))
